...
class WalletUsers(restful.Resource):
    ...
    def get(self, wallet_id):
        ...
        wallet.users.filter_by(user_id=user.id).first_or_404()
        return map(lambda x: \
                {'id': x.user_id, 'user_id': x.user_id, \
...
class Users(restful.Resource):
    ...
    update_user_parser.add_argument('token', type=str, required=True)
    ...
    def put(self):
        ...
        user = User.get_by_token(args['token'])
...
class EventList(restful.Resource):
    ...
    def post(self, wallet_id):
        ...
        if abs(total) > 0.01:
            return {'status': 'total money is not right'}
